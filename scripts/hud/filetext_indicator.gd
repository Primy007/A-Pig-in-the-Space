...
func _update_indicator():
	# Verifica se il file di testo e il player esistono ancora
	if !is_instance_valid(target_filetext) or !is_instance_valid(player):
		queue_free()
		return
	
	# Calcola se il file di testo è visibile sullo schermo
	var camera = get_viewport().get_camera_2d()
	if !camera:
		return
		
	# Converti la posizione del file di testo in coordinate schermo
	var filetext_screen_pos = _world_to_screen_position(target_filetext.global_position, camera)
	
	# Se il file di testo è visibile, nascondi l'indicatore
	if _is_filetext_on_screen(filetext_screen_pos):
		visible = false
		return
	else:
		visible = true
	...
	var indicator_pos = _calculate_edge_position(filetext_screen_pos)
	global_position = indicator_pos
	
	# Ruota la freccia verso il file di testo
	_rotate_arrow_to_filetext()
	
	# Aggiorna la distanza
	_update_distance()

func _is_filetext_on_screen(filetext_screen_pos: Vector2) -> bool:
	"""Verifica se il file di testo è visibile sullo schermo"""
	return filetext_screen_pos.x >= 0 and filetext_screen_pos.x <= screen_size.x and \
		   filetext_screen_pos.y >= 0 and filetext_screen_pos.y <= screen_size.y

func _calculate_edge_position(filetext_screen_pos: Vector2) -> Vector2:
	"""Calcola la posizione dell'indicatore sui bordi dello schermo"""
	var center = screen_size * 0.5
	var direction = (filetext_screen_pos - center).normalized()
...
func _rotate_arrow_to_filetext():
	"""Ruota la freccia per puntare verso il file di testo e posiziona l'icona dietro"""
	var direction_to_filetext = (target_filetext.global_position - player.global_position).normalized()
	arrow_sprite.rotation = direction_to_filetext.angle() + PI/2
	
	# Posiziona l'icona del file dietro la freccia (direzione opposta)
	var behind_direction = -direction_to_filetext
	file_icon.position = behind_direction * icon_distance_behind_arrow
